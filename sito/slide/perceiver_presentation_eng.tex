...
\begin{frame}[shrink=17]{Implementation Differences from the Paper}
  \vspace{0.15em}
  \begin{table}
    \centering\scriptsize
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.34\textwidth}>{\raggedright\arraybackslash}p{0.38\textwidth}}
      \toprule
      \textbf{Choice} & \textbf{Original paper} & \textbf{This project} \\
      \midrule
      Input tokenization &
      ...
      CIFAR-10 \textbf{identical}: $258$ dims $+$ 3 RGB $=261$. ModelNet40 and text: \textbf{$K=6$} ($3+39=42$ per point, $257+13=270$ per byte) \\
      \addlinespace[2pt]
      Augmentation, masking &
      ModelNet40: scale and shift jittered \emph{per point}. MLM: 15\% of \emph{whole words} masked &
      Scale and shift of the \emph{whole} cloud, undone by the normalisation. MLM: 15\% of \emph{single bytes} --- an easier task \\
      \addlinespace[2pt]
      Weight sharing &
      ...
      Model selection &
      --- &
      \textbf{Validation carved from train} (45k/5k/10k); test touched once. ModelNet40 has no validation split: epoch picked on test \\
      \addlinespace[2pt]
      Parameters &
      $30$--$200\text{M}+$ &
...
% ── The noise band ───────────────────────────────────────────────────────────
\begin{frame}[shrink=11]{The Noise Band: How Much Does a Number Move on Its Own?}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \vspace{0.2em}
      \texttt{e01} is the \emph{luckiest} of the three seeds. Measured from their mean (70.48\%) instead, \texttt{e12} and \texttt{e14} fall back inside: 10 runs clear the band, not 12.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=12]{CIFAR-10: What Survives the Noise Band}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.27\textwidth}
      ...
      {\scriptsize 10 against the three-seed mean}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \colhead{The one conclusive result}
        \small
        At $T=4$, interleaving beats front-loading by \textbf{4.44pp} --- outside the band, even measured from the three-seed mean ($3.29$pp). Re-reading the input \emph{after} the latents have been refined lets each pass look for something different.
    ...
    \begin{column}{0.44\textwidth}
      \begin{table}
        ...
        \begin{tabular}{ll}
          ...
          Input per point & 42 dims ($3 + 39$) \\
          Fourier bands / max f & \textbf{6} / 1120 (paper: 64) \\
      ...
      \vspace{0.2em}
      \scriptsize \textbf{Metric note:} ModelNet40 has no separate validation split --- the standard test set (2\,468 objects) \emph{is} the validation set, so the figure is \texttt{val\_accuracy} at the best epoch. The last epoch, with no selection at all, gives \textbf{86.43\%}.
    \end{column}
    \begin{column}{0.52\textwidth}
      ...
      \colhead{Above the paper}
        \small
        \textbf{87.36\%} vs the paper's \textbf{85.7\%}, with a batch $16\times$ smaller --- and still above at the last epoch. Plausible: ModelNet40 is small and its objects arrive canonically aligned, so the large-batch advantage that dominates ImageNet counts for little here.
...
\begin{frame}[shrink=5]{ModelNet40: Rotation Costs 13 Points --- and Why}
  \lead{Rotation costs 13.29 points — the positional encoding again. Scale and translation never reach the network.}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \begin{table}
        \centering\small
      ...
      \vspace{0.4em}
      \colhead{$^{*}$\,Two of the three runs see the same data}
        \small
        The normalisation re-centres each cloud and rescales it to unit radius, so a scale or a shift of the \emph{whole} cloud is undone before the model sees it --- the paper jitters \emph{each point} instead. \texttt{mn01} and \texttt{mn03} are therefore a free replica: $0.16$pp is seed noise.
    ...
    \begin{column}{0.50\textwidth}
      \colhead{The Perceiver has no rotational inductive bias}
        ...
        \vspace{0.4em}
        Here the axis is arbitrary in 3D and the rotation is applied in training only: the test objects stay upright. The paper also found that rotation did not help.
...
% ── MLM ──────────────────────────────────────────────────────────────────────
\begin{frame}[shrink=18]{WikiText-103: Byte-Level Masked Language Model}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \begin{table}
        ...
        \begin{tabular}{ll}
          ...
          Lookup of the two neighbours & 42.96\% \\
          Always a space (most frequent) & 19.07\% \\
          Uniform chance ($1/256$) & 0.39\% \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
    \begin{column}{0.50\textwidth}
      \colhead{Where the decoder finally earns its keep}
        \small
        Nearly nine masked bytes out of ten, reading raw UTF-8 with \textbf{no tokenizer at all} --- \textbf{44 points above} a lookup table of the two neighbouring bytes. That gap is what the long context buys.
      ...
      \alerthead{Read this number carefully}
        \scriptsize
        We mask \emph{single bytes}, the paper \emph{whole words}: a masked byte usually keeps its neighbours, which is why the lookup already reaches 42.96\% (\texttt{baseline\_mlm.py}). Nor is it comparable to masked-\emph{token} accuracy --- the comparison with BERT lives on GLUE.
...
\begin{frame}[shrink=26]{GLUE Multitask: the Comparison That Replicates Best}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \vspace{0.3em}
      \colhead{Where the gain sits}
        \small
        All on the small tasks --- STS-B $+16.9$, RTE $+7.6$, MRPC $+6.1$ --- which see only 1.35 passes over their own data. The large ones lose 2 to 3.6 points; CoLA stays at the majority class in both.
...
\begin{frame}[shrink=28]{Comprehensive Results Summary}
  ...
  \begin{table}
    ...
    \begin{tabular}{llccl}
      ...
      Point Clouds (ModelNet40) & Perceiver & \textbf{87.36\%} & 85.7\% & 3 \\
      Text (WikiText-103) & Perceiver IO & \textbf{86.68\%} MLM & 42.96\% lookup & 1 \\
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\small
        \item ModelNet40 \textbf{beats} the paper: $+$1.66pp ($+$0.73 at the last epoch)
        \item Byte-level MLM: 86.68\%, 44pp above a neighbour lookup
        ...
        \item 12 of 24 CIFAR runs are conclusive (10 against the seed mean)
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}[shrink=28]{Gap vs Original Papers}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      With a batch $16\times$ smaller (32 vs 512). Plausible because ModelNet40 is small and canonically aligned, so the large-batch advantage counts for little. The epoch is picked on the test set; the last one, 86.43\%, is still above.
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}[shrink=29]{Empirical Summary}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item ModelNet40 translation: $-0.16$pp --- undone by the normalisation, a free replica
      \end{itemize}
  ...
  \colhead{Central methodological finding}
    Measuring the \textbf{noise band} --- three replicas differing only by seed --- cost two runs and changed the status of all the others. Without it, 12 of the 24 comparisons (14 against the seed mean) would have been reported as effects when they are variance.
